...
function update_traffic_situation(detected_results) {
    for(var result_key in detected_results) {
        if (typeof(polylines[result_key]) == "undefined") {
            polylines[result_key] = L.polyline([detected_results[result_key][1],detected_results[result_key][2]], {weight: 10, color:'greed'})
            .on('click', function(e){ alertClick(e); }, {camera_name: result_key})
            .addTo(map_for_traffic_situation)
        }

        if (detected_results[result_key][0] > 4) {
            polylines[result_key].setStyle({color: 'red'})
        }
        else if (detected_results[result_key][0] > 2) {
            polylines[result_key].setStyle({color: 'orange'})
        }
        else {
            polylines[result_key].setStyle({color: 'green'})
        }
    }
}

$(window).on('load', function() {
    var map_for_traffic_situation = window["Object"].values(window)[window["Object"].keys(window).indexOf(leaflet_map_id)]
